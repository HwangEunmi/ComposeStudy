...
private fun TutorialContent() {
    ...
    Column(
        ...
        StyleableTutorialText(
            text = "@Immutable 또는 @Stable 타입인 경우, 리컴포지션을 건너띌 수 있다." +
                    "그러나 UnStable한 값들은 입력이 변경되지 않더라도 재구성된다.",
            bullets = false
        )
        ComposeScopeSample()
    }
...
private fun ComposeScopeSample() {
    ...
    Column {
        ...
        OuterComposable {
            // 이 영역은 리컴포지션 되지 않는다. (여기서 값을 read 하는 것이 없으므로)
            println("Outer SCOPE")
            MiddleComposable {
                // 이 영역에서 counter값을 읽기 때문에 이 영역은 재구성된다.
                println("Middle SCOPE")
                // Unstable 함수는 입력이 변경되지 않더라도 재구성된다.
